...
class ObjectTestModel(db.Model):
    __tablename__ = "test_models"
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(80), unique=True, nullable=False)


class ObjectTestService(BaseService):
    model = ObjectTestModel
...
def sample_record(app):
    ...
    record = ObjectTestModel(name="Sample")
    db.session.add(record)
    db.session.commit()
    return record

def test_create_success(app):
    result = ObjectTestService.create(name="Firewall A")
    assert result.id is not None
    assert result.name == "Firewall A"
    assert db.session.get(ObjectTestModel, result.id) is not None


def test_create_missing_unique_field(app):
    with pytest.raises(HTTPException) as e:
        ObjectTestService.create()
    assert e.value.code == 400


def test_create_duplicate_name(app, sample_record):
    with pytest.raises(HTTPException) as e:
        ObjectTestService.create(name="Sample")
    assert e.value.code == 409


def test_get_existing_record(app, sample_record):
    result = ObjectTestService.get(sample_record.id)
    assert result.name == "Sample"


def test_get_not_found(app):
    with pytest.raises(HTTPException) as e:
        ObjectTestService.get(999)
    assert e.value.code == 404


def test_update_success(app, sample_record):
    updated = ObjectTestService.update(sample_record.id, name="Updated")
    assert updated.name == "Updated"
    assert db.session.get(ObjectTestModel, sample_record.id).name == "Updated"


def test_update_empty_name(app, sample_record):
    with pytest.raises(HTTPException) as e:
        ObjectTestService.update(sample_record.id, name="   ")
    assert e.value.code == 400


def test_update_duplicate_name(app, sample_record):
    second = ObjectTestModel(name="Second")
    db.session.add(second)
    db.session.commit()
    with pytest.raises(HTTPException) as e:
        ObjectTestService.update(second.id, name="Sample")
    assert e.value.code == 409


def test_delete_success(app, sample_record):
    response = ObjectTestService.delete(sample_record.id)
    assert response == {"message": "Test_model deleted successfully"}
    assert db.session.get(ObjectTestModel, sample_record.id) is None


def test_delete_not_found(app):
    with pytest.raises(HTTPException) as e:
        ObjectTestService.delete(999)
    assert e.value.code == 404
...
def test_list_with_invalid_filter(app):
    with pytest.raises(HTTPException) as e:
        ObjectTestService.list(filters={"invalid": "x"})
    assert e.value.code == 400
